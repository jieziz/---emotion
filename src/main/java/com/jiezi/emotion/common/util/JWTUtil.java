...
public class JWTUtil {
    ...
    public static boolean verify(String token) {
        try {
            // 帐号加JWT私钥解密
            String secret = getClaim(token, Constants.ACCOUNT) + EncryptionUtil.decode(encryptJWTKey);
    ...
    /**
     * 生成签名token，用的是accessTokenExpireTime的时间，会存在比较短的时间
     * @param account 帐号
    ...
    public static String sign(String account, String currentTimeMillis) {
        try {
            // 帐号加JWT私钥加密
            String secret = account + EncryptionUtil.decode(encryptJWTKey);
